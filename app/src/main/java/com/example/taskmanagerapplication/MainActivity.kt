        ...
        viewButton.setOnClickListener {
            supportFragmentManager.beginTransaction().apply {
                ...
                if (todoRB.isChecked || deletedRB.isChecked) {
                    ...
                    if (!tasks.isEmpty()) {
                        /*(var) because it will be referencing a new String (which is the returned String from ( text = text + ("${it.name()}\t${it.priority()}\n")  )   )
                    and String is immutable. (each item while iterating by forEach on in tasks List it will take the name and priority and add it to the existing String. So, it's a new String).
                    String Pool now have a new String(after using concatenation)
                    in addition to the old String (before using concatenation)
                    */
                        var text = ""
                        tasks.forEach {
                            text = text + ("${it.name()} \t ${it.priority()}\n")
                        }
                        viewTasksTextView.setText(text)
                    }
                }
